...
const preloadPath = join(import.meta.dirname, 'preload.mjs');
const appPath = join(import.meta.dirname, 'app.mjs');

test('preload-import: register() works via --import preload', (t) => {
  t.plan(1);
  // The load hook replaces node:os with a synthetic module.
  // If the hook didn't register, the assertion would fail.
  const stdout = execFileSync(process.execPath, ['--import', preloadPath, appPath], {
    encoding: 'utf-8',
    timeout: 15_000,
  });
